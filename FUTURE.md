...
The app shipped publicly as v1.0.0-rc.1+ and is functionally complete. These
are the polish items that separate "RC quality" from a confident GA tag.
None are strictly blocking — but earning the 1.0.0 (no -rc) drop should
clear at least the test suite + accessibility cleanup items.

### Test coverage (highest leverage)

LiftTrace has **zero automated test coverage** today. Every release is
"build, hope, ship." This is the single biggest GA blocker.
...
- **Commercial dataset snapshot** — buying the v1 dataset outright from AscendAPI's shop so we can bundle rich GIFs in the paid APK without an ongoing API dependency. One-time cost vs. subscription.
