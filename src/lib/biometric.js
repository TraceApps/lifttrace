 ...
 * stored JWT lives in localStorage which is already encrypted at rest by
 * Android File-Based Encryption (locked phone == encrypted disk since
 * Android 7). Biometric verification just gates unlocking that saved token
 * instead of the user retyping their password each time.
 *
 * Lifecycle:
...
const TOKEN_KEY = 'lt:biometric:token';
...
export async function authenticate(reason = 'Sign in to LiftTrace') {
  const p = _getPlugin();
  if (!p) return false;
  try {
