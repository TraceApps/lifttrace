 ...
 * browser / WebView console is also captured in a buffer the user can view,
 * copy, and share via Settings → Diagnostics.
 *
 ...
 *
 * Verbose mode: when localStorage `lt:verboseLogging === '1'`, the
 * file-local `_dlog` helpers in db-native.js / sync.js / notifications.js /
 * etc. start logging too (they normally only run in dev builds). Lets users
 * grab high-detail logs while reproducing a bug, without forcing verbose
 * logging on everyone.
 *
...
// Catch uncaught errors + unhandled promise rejections — the most useful
// entries to have in a bug report.
...
/** Returns the buffer joined into a single text blob (for clipboard / share). */
...
export function isVerboseLogging() {
  try { return localStorage.getItem('lt:verboseLogging') === '1'; } catch { return false; }
}

/** Toggle verbose mode. Persists across reloads. */
export function setVerboseLogging(on) {
  try {
    if (on) localStorage.setItem('lt:verboseLogging', '1');
    else    localStorage.removeItem('lt:verboseLogging');
  } catch {}
}
