<script>
   ...
   * user's timeFormat (12h/24h) setting.
   *
   * TraceApps parity: byte-for-byte port from NutriTrace's TimePicker so
   * the shared component story holds across NT + CT + LT. When updating
   * one copy, mirror to the others; treat divergence as a bug.
   *
   * Props:
   *   value (bindable) — HH:MM (24h internal)
   *   label            — optional label rendered inside the trigger button
...
<style>
  ...
  .tp-trigger:disabled { opacity: 0.55; cursor: not-allowed; }
