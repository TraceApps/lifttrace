<script>
  ...
  // ────────────────────────────────────────────────────────────────────────
  // ⚠ KEEP IN LOCKSTEP WITH NUTRITRACE
  //   sister file: ../../../../nutritrace/src/components/settings/SettingsAuth.svelte
  ...
  //   here, mirror the change to NutriTrace's copy in the same commit.
  ...
  const PROVIDER_PRESETS = [
    {
      ...
      help: 'Auth0 adds custom claims under a namespaced URL like "https://lifttrace.app/roles" — leave the admin claim blank for now and contact your tenant admin to set up a rule that exposes role membership.',
      hides: [],
    },
    {
      id: 'authelia',
  ...
  // LiftTrace's Settings.svelte sub-component contract (visible / expanded /
  // onToggle), so this section renders its own toggle button instead of being
  // mounted body-only the way NutriTrace does it.
