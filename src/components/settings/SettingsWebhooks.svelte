<script>
   ...
   *
   * Admin-only Settings section for outgoing webhook management (issue
   * #79). Lists existing webhooks, lets the admin create new ones (with
   * a target URL + event checkboxes + optional custom secret), test one
   * without waiting for a real event, and delete them.
   *
  ...
  let newUrl = '';
  let newEvents = new Set(['workout.completed']);
  ...
  async function createNewWebhook() {
    ...
    try {
      ...
      newUrl = '';
      newEvents = new Set(['workout.completed']);
      newSecret = '';
      await load();
    } catch (e) {
      showError(e.message);
  ...
  async function deleteOne(w) {
    ...
    try {
      const r = await fetch(apiUrl(`/api/webhooks/${w.id}`), {
  ...
  async function testOne(w) {
    ...
    testingId = w.id;
    try {
      const r = await fetch(apiUrl(`/api/webhooks/${w.id}/test`), {
