<script>
   ...
   *
   * Admin-only Settings section for MCP personal access token management
   * (issue #78). Lists existing tokens, lets the admin create new ones
   * (with name + scope checkboxes + optional expiry), and revoke them.
   *
   ...
   * that the server only stores a SHA-256 hash. Ported from
   * NutriTrace's SettingsApiTokens.svelte (issue #103).
   */
  ...
  // LT sub-components are body-only — the section-toggle button is
  // rendered externally in Settings.svelte. This component only renders
  // its body, gated by `expanded` from the parent (matches
  // SettingsAuth / SettingsBackup / SettingsUserManagement).
          ...
          {:else}
            <div class="create-form" transition:slide={{ duration: 160 }}>
              ...
              <div class="form-group">
                ...
                {:else}
                  <div class="mcp-status">
                    {$_('settings_api_tokens.form.mcp_disabled')}
                  </div>
                {/if}
