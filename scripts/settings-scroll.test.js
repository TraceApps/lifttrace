/**
 * /settings and /settings/<section> share the page's scroll (here the
 * document's), so a section opened wherever the index was scrolled to (same
 * as NutriTrace #227). A section now opens
...
test('the index and its sections share one scroller (why the fix is needed)', () => {
  ...
  assert.match(app, /'\/settings\/:section':\s+Settings,/);
...
test('a section opens at its top; the index returns to where it was', () => {
  assert.match(settings, /document\.scrollingElement/, 'the document scrolls in LiftTrace');
...
test('the index position is recorded while scrolling, not on the way out', () => {
  ...
  assert.match(settings, /window\.addEventListener\('scroll', _recordIndexScroll/);
  assert.match(settings, /window\.removeEventListener\('scroll', _recordIndexScroll\)/);
  assert.match(settings, /function _recordIndexScroll\(\) \{\s*if \(_onIndexUrl\(\)\) _scrollMemo\.indexTop = _docScroller\(\)\?\.scrollTop \|\| 0;/);
});
