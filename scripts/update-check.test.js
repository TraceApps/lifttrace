...
test('setup asks, with the toggle off to begin with', () => {
  assert.match(wizard, /const STEPS = \[[^\]]*'updates'\]/, 'the step list ends with the question');
  assert.match(wizard, /let updateChecks = false;/);
  assert.match(wizard, /async function finish\(\) \{\s*await _saveUpdateChoice\(\);/, 'answered before the wizard exits');
});
