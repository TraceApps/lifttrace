/**
 * Behavioral tests for the outgoing webhook delivery mechanics (issue
 * #79): signing and the actual HTTP send. server/lib/webhook-delivery.js
 * deliberately has no db.js import (server/lib/webhooks.js owns the
 * DB-backed subscription management and imports these two functions
 * from it), so this runs without a compiled better-sqlite3 native
 * binding, unlike the rest of the webhooks feature (dispatchWebhookEvent,
 * the CRUD helpers), which read/write the webhooks table and can only
 * be verified against a running dev server or in CI with the real
 * binding.
 */
...
test('signEnvelope produces a JSON envelope with event/timestamp/data and a matching HMAC-SHA256 signature', () => {
  const { envelope, signature } = signEnvelope('my-secret', 'workout.completed', { date: '2026-09-12' });
  const parsed = JSON.parse(envelope);
  assert.equal(parsed.event, 'workout.completed');
...
test('signEnvelope is deterministic for the same input at the same instant but signature changes with the secret', () => {
  const a = signEnvelope('secret-a', 'pr.set', { exercise_id: 1 });
  const b = signEnvelope('secret-b', 'pr.set', { exercise_id: 1 });
  ...
  const { url, close } = await withServer((req, res) => {
    ...
    req.on('end', () => {
      received = {
        method: req.method,
        headers: {
          signature: req.headers['x-lifttrace-signature'],
          event: req.headers['x-lifttrace-event'],
          delivery: req.headers['x-lifttrace-delivery'],
...
test('sendWebhookRequest throws on a non-2xx response, so the retry loop in _deliverWithRetry actually retries', async () => {
  ...
  try {
    ...
    await assert.rejects(() => sendWebhookRequest(url, envelope, signature, 'd-1', 'workout.completed'), /500/);
  } finally {
    await close();
  }
});
