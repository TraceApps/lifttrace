/**
 * Every surface you cannot move mid-set has a rule that moves it off the
 * crease.
 *
 * A diary, a chart or a photo is free to cross a fold: it can be scrolled or
 * panned. A dialog, a sheet, the rest timer or the Trace panel cannot, so each
 * one is named here, and a new one gets noticed when it is added rather than
 * when someone unfolds a phone in a gym.
 */
...
test('the book rules cover every fixed surface', () => {
  for (const sel of ['.dialog-backdrop', '.sheet-backdrop', '.as-backdrop', '.lb-panel', '.rest-sticky']) {
    assert.match(css, new RegExp(`html\\.fold-book [^{]*\\${sel}`), `book: ${sel}`);
  }
});

test('the tabletop rules cover every fixed surface', () => {
  for (const sel of ['.dialog-backdrop', '.sheet-panel', '.as-panel', '.lb-panel']) {
