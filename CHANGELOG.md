...
- **Progress photos.** Attach dated photos to your body-stats history and watch yourself change over time. Lives on its own page at `/progress`, reached from Statistics (next to the body-weight chart) and from the Body Stats sheet, which gains an "Add Photo" button so a photo can be logged alongside the day's measurements. Three ways to look at them:
  ...
  - The **scrubber** opens when you tap a photo: full frame, with a draggable date track underneath, or press play and watch the whole record run as a time-lapse. The track is a true date axis rather than one even step per photo, so a gap in the habit shows up as a gap. Compare answers how two dates differ; this answers what the whole arc looked like, which matters because any two adjacent photos look identical and the change is only visible in the sweep.

  Weight for a photo's date shows with its unit and, in the scrubber, the change since your first weighed photo. If a date has no weight logged you are offered one inline rather than being sent to the diary, writing to the same body-stats row every other weight surface uses (merged into that day, so a waist or body-fat figure already recorded is never clobbered). Captures default to today but the date is settable, so an old camera roll can be backfilled without walking the diary a day at a time.

  iPhone HEIC photos are converted to JPEG in the browser before upload, since Chrome, Firefox and Android WebView cannot display HEIC at all; the converter loads on demand, so it costs nothing for anyone who never picks one. Photos sync across devices, are included in full backups (rows and files), and are deleted from disk, not just from the database, when a photo, a user's data, or an account is removed, which is a gap the app's other uploads still have. Photos never expire: they are the long-term record, which is the opposite of what a coaching clip ([#57](https://github.com/TraceApps/lifttrace/issues/57)) would want, so the two stay separate features rather than sharing a lifetime policy.

  Also exposed through the MCP server (`list_progress_photos`, `add_progress_photo`) and the REST API (`GET`/`POST /api/v1/body-stats/photos`) under the existing `mcp:read`/`mcp:write` scopes, with a new `progress_photo.logged` webhook that fires no matter which of the three attaches the photo. Unlike avatars and exercise media, photo files are not public by URL; see Security below.
...
- **Outgoing webhooks** ([#79](https://github.com/TraceApps/lifttrace/issues/79), requested by @bursaar). New Settings, Webhooks section: point LiftTrace at a URL and it fires a signed HTTP POST the instant a workout is completed, a personal record is set, an active program advances to its next week, or a body stat is logged. The push counterpart to the REST API above, for wiring into n8n, Home Assistant, or any other self-hosted automation without polling. Off by default (`WEBHOOKS_ENABLED=1` to turn on). Each delivery is signed with HMAC-SHA256 over the request body (`X-LiftTrace-Signature`), retried up to 3 times on failure, and the shared secret is encrypted at rest, shown once at creation like an API token. Target URLs are validated against the same SSRF guard already protecting the radio proxy (link-local, cloud-metadata, and private/loopback addresses blocked by default), extracted into a shared module in this release so both features stay in sync rather than carrying separate copies; the check re-runs on every retry attempt and redirects are not followed, so a target cannot pass validation once and then redirect a later attempt somewhere internal. Personal-record detection is new server-side logic (LiftTrace previously only computed PRs client-side for the in-app celebration toast); it reuses the exact record definition the Statistics page and `/api/v1/records` already use, so all three agree on what counts as a record. A per-webhook "send test event" button verifies a target works without waiting for a real one. See `docs/webhooks.md`.

- **Trace can see your cardio, and log it.** Cardio has its own table, so nothing in `get_workouts` or `get_stats_overview` ever reflected it: Trace could tell someone training five days a week that they had done nothing since Tuesday, and would try to record a run as a lifting session. New `get_cardio` (sessions, distance, average heart rate, totals per activity) and `log_cardio`, with the system prompt told outright that cardio is absent from the lifting tools, since otherwise a model has no reason to go looking. A test now maps every user-data table to a tool or an explicit exemption, so a future feature that ships its own table cannot go unnoticed by the coach the way this one did.

- **Trace can see your progress photo history.** New `get_progress_photos`, so it can answer consistency questions ("how often am I taking photos?", "when was my last one?") and tie the photo record to the weight logged on those dates. Metadata only: dates, counts and weights, never image content. Sending your body photos to whichever AI provider you have configured is not something a convenience tool should do as a side effect, so if you want Trace to look at one you attach it to a message yourself, which it has always supported.

### Changed

- **Muscle recovery body map, redrawn.** The old figure was built on wrong proportions: its hip sat 71% of the way down the body instead of 50%, so the legs were about half the length they should be, and its shoulders and waist were the same width. A torso that long with no taper reads as a mannequin in a skirt whatever colour the muscles are. The figure now follows the standard eight-head canon with a real V-taper, muscle regions are anatomical shapes rather than rectangles, and each is clipped to the silhouette so it finishes flush with the body edge instead of leaving the offset slivers you could see before. Arms attach at the shoulder, the knee and calf exist, and so do feet.

### Fixed

- **The muscle recovery map was blank for anyone with no recent training.** An untrained muscle was drawn in `--surface-2`, all but identical to the silhouette fill, so with no completed sets in the window every region disappeared and the card showed a featureless dark shape rather than a body map. That is the first thing a new user sees, so it now renders as a visible region.

### Security

- **Progress photos are no longer readable by URL.** Files under `UPLOADS_PATH/body-stats/` are excluded from the static handler and served only through `GET /api/body-stats/photos/:id/file` (or `GET /api/v1/body-stats/photos/:id/file` for API tokens), which verify the row belongs to the requester, so one account cannot read another's photos either. `/uploads` is mounted ahead of the auth middleware so an Android WebView `<img>` can load images without an `Authorization` header, which is a fine trade for a shared exercise-demo GIF and a bad one for a progress photo; the app now fetches photo bytes with an ordinary `fetch()` (which can carry a session where an `<img>` request cannot) and renders them from an object URL. The exclusion tests the resolved filesystem path rather than the request URL: the obvious version, a route prefix on `/uploads/body-stats`, is bypassable, because `express.static` percent-decodes a path before opening the file while a router prefix matches the raw one. `/uploads/%62ody-stats/x.jpg`, `/uploads/body%2Dstats/x.jpg` and `/uploads//body-stats/x.jpg` each served a real file against that version. Avatars and exercise media are unchanged and stay public by URL. Note that single-user mode has no accounts and therefore no identity to check against, so the whole app remains open by design there, the same as every other endpoint; put it behind a reverse proxy or a VPN, as always.

- **Backup archives are no longer reachable from the public uploads directory.** `BACKUPS_PATH` defaults to a directory inside `UPLOADS_PATH`, which is served ahead of the auth middleware, so a full-backup ZIP sitting there was fetchable by URL while every `/api/full-backup` route is admin-only. It now returns 404 like anything else outside the served set. Scheduled backups are off by default, so an install that never enabled them and never created one by hand had nothing there to reach; there is no directory listing either, so a filename had to be known or guessed. The archive holds a full database dump, so if yours has been internet-facing with backups enabled, a look through your access log for `/uploads/backups/` will settle it either way.
