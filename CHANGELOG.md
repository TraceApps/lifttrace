...
- **The Android back button closes what's open first.** Back only went back a page, so with a sheet, dialog, menu or picker open it left the page underneath with it still showing. Back now closes the newest sheet, dialog, menu, number or RPE picker, the full-screen player or the Trace chat first, one at a time, the same as its own close button (a dialog closes as Cancel). The sync merge questions still need an answer, so back leaves them open. It also closes the slide-out sidebar if that's showing. With nothing open, back goes back a page, as before.
- **A Settings category now opens at its top.** Picking a category from lower down the Settings list opened it partway down, because the category kept the list's scroll position. Categories now open at their top, and going back to the list returns you to where you were on it. A setting found through Settings search still scrolls straight to its match. Same fix as NutriTrace [#227](https://github.com/TraceApps/nutritrace/issues/227).
- **Body Stats no longer slides under the status bar.** In the Android app, Body Stats brings up the keyboard, and the sheet grew taller than the space above it, so its title and close button ended up under the status bar where they couldn't be tapped. The sheet now stays below the status bar and scrolls its fields instead, with the title and close button always in reach. Gym Tools and the Diary date picker got the same limit. Same fix as NutriTrace [#228](https://github.com/TraceApps/nutritrace/issues/228).
- **Other sheets stay below the status bar too.** The shared sheet used across the app, the workout summary and Smart Log. The Android app draws under the status bar, and these were capped only at a share of the screen, so one that filled its cap (a tall one, or any with the keyboard up) could start under the status bar. They now always stop below it and scroll their content instead. Nothing changes where there's room, or on a computer.
- **The bottom of the side columns was hidden behind the Radio player** ([#104](https://github.com/TraceApps/lifttrace/issues/104), reported by @LeVraiRoiDHyrule). On wider screens, the columns beside the main page (the Diary's workout panel, and those on Exercises, Programs, Statistics, Radio and Settings) only left room for the nav bar, so while Radio was playing the mini player covered their last row and it couldn't be scrolled into view; in the Diary that was Delete Workout. The rest timer and hold timer bars covered them the same way. These columns now leave room for whatever is showing at the bottom of the screen, and look exactly as before when nothing is.
- **Auto-Fill Last Weights could skip your last session** ([#103](https://github.com/TraceApps/lifttrace/issues/103), reported and diagnosed by @kgenerozov). Loading a template, adding an exercise or loading a recent workout only looked at the newest workout that exercise appeared in, so if that one had nothing ticked (today's unfinished workout, say), the sets came up with template values even though the Last row showed your previous numbers. Auto-fill now uses the last session where you completed working sets, the same one the Last row shows. Warm-up sets are no longer copied as working sets: a 45 lb warm-up last time used to become the weight of your first working set when loading a template. A template's own warm-up rows now fill from last time's warm-ups, and its working rows from last time's working sets. Adding a single exercise or loading a recent workout still brings its warm-ups back, now marked as warm-ups instead of counting toward volume and PRs. Per-set template targets and an active program week still take priority over history, as before.
- **Android: a workout disappeared from the Diary when the connection dropped** ([#102](https://github.com/TraceApps/lifttrace/issues/102), reported and diagnosed by @kgenerozov). Connected to a server, a save made without internet was kept on the phone and queued for later, but the app then treated the reply as an empty workout and cleared the Diary; this also hit starting a new session offline and saves made as the app went into the background. The workout now stays on screen and the edit is kept. Two related problems are fixed with it: an offline edit left that day's copy on the phone stuck, so later changes made on the web never reached it, and a save made online only reached the phone's own copy at the next sync, so reopening the day offline could show an older version. Once offline edits reach the server, the phone's copy of that day is replaced with the server's, which also avoids a duplicate session when a workout was started offline; edits to a second session started offline stay on that session when they sync, and deleting one before it syncs keeps it deleted; if a queued save has to wait for a retry, later saves to the same day wait behind it instead of reaching the server out of order; and every save made online updates the phone's copy right away, without slowing the save down.
...
- **The weekly summary says how the week actually went** ([#98](https://github.com/TraceApps/lifttrace/issues/98), requested by @backmind). It used to be a workout count and a volume number with no unit. The email now shows sessions against your weekly goal, working sets, volume with its unit, time trained and personal records, each compared with your average week over the month before, a list of the week's new records (holds included), and your sets by muscle group as simple bars that display in any mail client. A View This Week button opens Statistics on the last seven days. The push notification gets the same in one line, such as "4 of 5 sessions, 62 sets, 3 PRs, 48,200 lbs volume (+8% vs your 4-week average)". Numbers are formatted for your language.

### Fixed

- **Radio couldn't play from Jellyfin 12** ([#100](https://github.com/TraceApps/lifttrace/issues/100), reported by @LeVraiRoiDHyrule). Jellyfin 12 no longer accepts the older way LiftTrace signed in, so every library request was refused with a 401, while the Test button in Settings still said the connection worked because it only read the server's public info page. Radio now signs in the way current Jellyfin expects, and falls back to the older method if a server still needs it, so earlier Jellyfin versions keep working. The Test button now signs in for real, so a wrong password shows up there; a saved sign-in that stops working (new server, changed password) is renewed automatically instead of failing until you reconnect; and a refused sign-in says to check the username and password rather than showing a bare error code.
- **A workout couldn't be added back after Clear Workout** ([#99](https://github.com/TraceApps/lifttrace/issues/99), reported by @LeVraiRoiDHyrule). Loading a template copied the template's own exercise ids into the day, and Clear Workout records the ids it removes so other devices remove them too. Loading the same template again sent those same ids, so they were treated as already deleted and dropped: the save appeared to work and the day stayed empty. Taking one exercise out and reloading the template lost it the same way. Exercises copied into a day now get ids of their own, whether they come from a template, a prescription, yesterday or a recent workout, so they can be added back as often as you like. Days already affected work again without any change to your data.
- **The weekly summary counted the wrong days.** It went out on the server's weekday rather than yours, so it could arrive a day early or late for anyone in a different time zone; its "week" covered eight days; days you opened but never trained counted as workouts; and the PRs its notification promised were never counted.
- **Android: tapping a weight or reps field brought up the system text toolbar over the set row** ([#95](https://github.com/TraceApps/lifttrace/issues/95), reported and diagnosed by @kgenerozov). Tapping a number selected it so that typing would replace it, and on Android any selected text brings up the Translate, Cut and Copy toolbar, right over the value you were about to retype. Nothing is selected now: the value dims when you tap, and the first digit you type replaces it, so tap-and-type works the same with no toolbar. Backspace, a second tap to place the cursor, and the arrow keys still edit the value normally. This covers weight, reps, left and right reps, hold time and the Diary's body-weight field.
- **Taking an exercise out of a superset made the whole superset disappear from the Diary** ([#96](https://github.com/TraceApps/lifttrace/pull/96), found and fixed by @backmind). A superset is drawn from a run of consecutive members, but removing one from the middle left it sitting exactly where it was, between the members it had just left. The two that stayed were no longer next to each other, so each rendered as an ordinary exercise card and the superset vanished from the screen, while the workout still held them as one superset (set completion still gated them as a round, and the rest timer still waited for both). Getting the block back meant adding the exercise in again, moving it down, and taking it out a second time. An exercise leaving a superset now moves below the block, so the members left behind stay together; leaving from the top or the bottom keeps its place exactly as before. Same treatment in the program day editor, which groups and removes the same way.
- **Android: reminders ignored the times set in Settings, the streak reminder never fired, and the weekly summary arrived every day** ([#97](https://github.com/TraceApps/lifttrace/pull/97), found and fixed by @backmind). The on-device reminders read setting names the app never stores (`notifWorkoutReminderTime`, `notifStreakAtRisk`, `notifStreakAtRiskTime`, `notifWeeklySummaryTime` and `notifWeeklySummaryDay`), while the settings layer and the server scheduler both write `notifWorkoutTime`, `notifStreakAlert`, `notifStreakTime`, `weeklySummaryTime` and `weeklySummaryDay`. Every one of those reads fell back to a hardcoded default, so the workout reminder fired at 17:00 whatever time you had picked, the streak reminder never fired at all because even its on/off switch was read under a name nothing writes, and the weekly summary was scheduled as a daily notification at 18:00, with the chosen day never consulted. On top of that, the reminders were set as plain repeating alarms counted from the moment they were scheduled rather than at a time of day, and the app reschedules them every time it starts, so opening the app kept pushing the next one a full day out. They now read the names the app actually writes, with the defaults the settings store uses, and they fire at the time of day you chose, the weekly summary once a week on the day you chose. The native WorkManager path, which takes over when `_USE_NATIVE_WORKER` is on, carried the same mismatch and gets the same names.

### Security

- Updated `devalue`, a library Svelte depends on, to clear a moderate advisory about malformed input causing a denial of service ([GHSA-9rgm-9g3h-6x36](https://github.com/advisories/GHSA-9rgm-9g3h-6x36)). LiftTrace doesn't pass untrusted input to it, so this is precautionary.
...
- **Timed sets for planks, holds and carries** ([#89](https://github.com/TraceApps/lifttrace/issues/89), requested by @chrisfeagles). Not every set is weight and reps: a plank, wall sit, dead hang or farmer's walk is tracked by how long you held it. An exercise can now be tracked by Time instead of Reps, and its set row shows a time field where reps would be, with weight still there for weighted holds (`25 lbs for 1:00`) or left empty for bodyweight. Typing a time works like a microwave or a phone timer: digits fill in from the right, so `45` is 0:45, `130` is 1:30 and `200` is 2:00, shown as you type.
  ...
  - **A hold timer, so you don't have to type the time at all.** A timed set has a timer button inside its time field. Tap it, get a three second count-in to get into position (tap the number to skip it), hold, and tap Stop: the set is filled in and ticked off exactly as if you had typed the time and tapped the checkbox, so rest timer, PR detection and supersets all behave as usual. The time shows large enough to read from the floor mid-plank. If the set already has a time on it (from a program, or your last session) you get a cue when you reach it, and the clock keeps running so you can go past it. It uses your rest timer sound and vibration settings, keeps the screen awake for the length of the hold, and keeps counting correctly if the phone locks or you leave the Diary.
  ...
  - **Everything else speaks it too.** Programs and templates can prescribe a time, per set or across a periodized weeks plan. Smart-Add understands `plank 3x45s`. CSV export gains a `duration_sec` column (added at the end, so spreadsheets that read columns by position keep working). Trace can log and read timed sets. Over MCP and the REST API, `log_set` takes `duration_sec`, `get_workout` returns it on each set, `search_exercises` reports each exercise's `set_type`, records carry `maxDuration`, and the `pr.set` webhook includes the hold record.
  - **Imports stop losing holds.** Strong, Hevy, FitNotes and Garmin all record hold durations, and every one of those importers was either ignoring that field or reading it and throwing it away, so a plank came in as an empty set. They now arrive with their times.
...
- **Trace said lifts were missing from your log when they weren't** ([#92](https://github.com/TraceApps/lifttrace/issues/92), reported and diagnosed by @kgenerozov). Asked to compare today's session with earlier ones, the AI model would often invent a date range in the wrong year (2024, with today in 2026), find nothing there, and tell you the exercise wasn't logged. Trace now checks its own date ranges: a range that finds nothing is retried with the normal window, and the model is told today's real date so it stops repeating the mistake. A lift you last trained more than 30 days ago is also found now instead of reported missing. The same guessing could have saved a workout, cardio session or body stat to a date two years back where you'd never find it, so Trace now refuses to write to a date more than 60 days from today until you confirm it. Separately, Trace was being told that "today" was whichever day the Diary happened to be showing, which invited this mistake whenever you had browsed to another date; it now gets the real date, and is told which day the Diary is open on.
...
- **Deleted workouts still counted in Statistics and personal records.** A deleted workout was hidden from the Diary but still counted everywhere numbers are built from your history, so its sets could hold a personal record, add to volume and charts, and push the "All" range back to its date. Records over MCP, the REST API and the `pr.set` webhook had the same gap, and so did workout, rest day and streak reminders (a deleted workout counted as training that day), the weekly summary, program week progress, exercise usage sorting, and the coach views of a member's recent workouts, streak and prescriptions. Deleted workouts are now left out of all of them, and re-importing a workout you deleted no longer skips it as a duplicate.
- **MCP: deleting a workout, then logging to its date, misbehaved.** `delete_workout` could pick an already-deleted workout on that date and report success while the real one stayed, and it removed rows outright, so synced devices never learned the workout was gone and could push it back. It now deletes the live workout the same way the app does. Logging a set to a date whose workout you had deleted also brought the whole deleted workout back, old sets and all; it now starts a fresh session.
...
- **Uploaded files are stored and served more strictly.** An upload's file extension now comes from what kind of file it actually is rather than the name it was uploaded with, and everything under `/uploads` is served with `X-Content-Type-Options: nosniff` and a sandboxing content policy. Images, audio and video display exactly as before.

## [1.3.0-dev05] - 2026-09-13

### Added

- **Progress photos.** Attach dated photos to your body-stats history and watch yourself change over time. Lives on its own page at `/progress`, reached from Statistics (next to the body-weight chart) and from the Body Stats sheet, which gains an "Add Photo" button so a photo can be logged alongside the day's measurements. Three ways to look at them:
  - The **timeline** groups by month and shows the weight logged that day beside each photo.
  - **Compare** stacks any two and wipes between them with a draggable divider, rather than shrinking both into a split view, so small changes still register. "Compare First and Latest" jumps straight to the comparison the feature exists for.
  - The **scrubber** opens when you tap a photo: full frame, with a draggable date track underneath, or press play and watch the whole record run as a time-lapse. The track is a true date axis rather than one even step per photo, so a gap in the habit shows up as a gap.

  Weight for a photo's date shows with its unit and, in the scrubber, the change since your first weighed photo. If a date has no weight logged you are offered one inline rather than being sent to the diary. Captures default to today but the date is settable, so an old camera roll can be backfilled without walking the diary a day at a time.

  Photo files are private: unlike avatars and exercise media they are not readable from `/uploads` by URL, and are served only to the account that owns them. In single-user mode there are no accounts and so no identity to check against, the same as every other endpoint; put it behind a reverse proxy or a VPN, as always.

  iPhone HEIC photos are converted to JPEG in the browser before upload, since Chrome, Firefox and Android WebView cannot display HEIC at all. Photos sync across devices, are included in full backups (rows and files), and are deleted from disk, not just from the database, when a photo, a user's data, or an account is removed. They never expire. Trace can read the history too (dates, counts and the weight logged on them, never the images themselves), and they are exposed through the MCP server and the REST API with a `progress_photo.logged` webhook.

- **Public REST API for external integrations** ([#77](https://github.com/TraceApps/lifttrace/issues/77), requested by @bursaar). A versioned REST API at `/api/v1`, off by default (`PUBLIC_API_ENABLED=1` to turn on), for scripts and automations that want plain JSON over HTTP rather than the MCP protocol (#78). Eight read routes covering workouts, recent workouts, personal records, exercise progress, exercise search, programs, the active program, and body stats, plus two additive write routes (log a set, log a body stat, gated on `PUBLIC_API_WRITE_ENABLED=1` in addition to the base flag, mirroring MCP's own read/write split). Uses the same personal access tokens and `mcp:read`/`mcp:write` scopes MCP already introduced, so one token works for both interfaces. Deletion stays MCP-only: it is the one irreversible operation in the set and this surface has not needed it. See `docs/public-api.md` for the full endpoint reference.

- **Outgoing webhooks** ([#79](https://github.com/TraceApps/lifttrace/issues/79), requested by @bursaar). New Settings, Webhooks section: point LiftTrace at a URL and it fires a signed HTTP POST the instant a workout is completed, a personal record is set, an active program advances to its next week, or a body stat is logged. The push counterpart to the REST API above, for wiring into n8n, Home Assistant, or any other self-hosted automation without polling. Off by default (`WEBHOOKS_ENABLED=1` to turn on). Each delivery is signed with HMAC-SHA256 over the request body (`X-LiftTrace-Signature`) and retried up to 3 times on failure, and the shared secret is encrypted at rest, shown once at creation like an API token. Target URLs are validated against the same SSRF guard protecting the radio proxy: link-local, cloud-metadata and private/loopback addresses are blocked by default, the check re-runs on every retry, and redirects are not followed. A per-webhook "send test event" button verifies a target works without waiting for a real one. See `docs/webhooks.md`.

- **Trace can see your cardio, and log it.** Cardio is stored separately from lifting, so nothing in Trace's workout tools ever reflected it: it could tell someone training five days a week that they had done nothing since Tuesday, and would try to record a run as a lifting session. It can now read your cardio sessions (distance, average heart rate, totals per activity) and log new ones.

### Changed

- **Muscle recovery body map, redrawn.** The old figure was built on wrong proportions: its hip sat 71% of the way down the body instead of 50%, so the legs were about half the length they should be, and its shoulders and waist were the same width. A torso that long with no taper reads as a mannequin in a skirt whatever colour the muscles are. The figure now follows the standard eight-head canon with a real V-taper, muscle regions are anatomical shapes rather than rectangles, and each is clipped to the silhouette so it finishes flush with the body edge. Arms attach at the shoulder, the knee and calf exist, and so do feet.

### Fixed

- **The muscle recovery map looked empty if you had not trained recently.** An untrained muscle was drawn in a colour all but identical to the body silhouette, so with no completed sets in the window every region disappeared and the card showed a featureless dark shape rather than a body map.

### Security

- **Full-backup archives were downloadable without signing in.** `BACKUPS_PATH` defaults to a directory inside `UPLOADS_PATH`, which is served ahead of the auth middleware so images can load without an `Authorization` header, so a backup ZIP sitting there was fetchable by URL even though every `/api/full-backup` route is admin-only. That directory is now excluded from the static handler. Scheduled backups are off by default, so an install that never enabled them and never created one by hand had nothing there to reach; there is no directory listing either, so a filename had to be known or guessed. The archive holds a full database dump, so if yours has been internet-facing with backups enabled, a look through your access log for `/uploads/backups/` will settle it either way.
...
- Bumped `multer` (2.2.0 to 2.3.0, three DoS/bypass advisories) and `nodemailer` (9.0.3 to 9.1.1, four advisories including a recipient-domain validation bypass) via a non-forcing `npm audit fix` within already-permitted lockfile ranges. `adm-zip`'s open moderate advisory (symlink-following during extraction) has no upstream patch yet and only offers a semver-major downgrade that isn't actually safer, so it's left as-is: every route touching it is admin-only, and the actual extraction code here never calls `adm-zip`'s own filesystem-writing methods, it only reads entry bytes via `getEntries()`/`entry.getData()` and writes them through its own path-validated `fs.writeFileSync`, so the vulnerable code path in the dependency is never reached regardless of version.

## [1.3.0-dev03] - 2026-09-07

### Fixed

- **Android: editing a set (weight, reps, or the completed checkbox) could revert a moment later, sometimes on every single edit.** `GET /api/workout/:date` is local-first: it always serves whatever the on-device SQLite cache currently holds, and that cache is only ever updated by a background pull applying the server's copy, never by this device's own saves (those update the on-screen state directly from the save's network response). Diary reloads today's workout on every `lt:sync-complete` event so a device that pulls a genuine change reflects it immediately, but that event fires roughly every few seconds while the app is open, and the overwhelming majority of those pulls touch nothing relevant. Under a slow or unstable connection, the on-device cache could lag well behind an edit that had already saved successfully, so the next one of these frequent reloads served the stale cached copy and visibly undid the edit. Now only reloads when the pull actually touched today's workout or a per-entry deletion, and refuses to apply anything older than what's already on screen for that same session. A pre-existing "stale request" guard nearby turned out to compare a variable to a copy of itself, always true to proceed, never actually catching anything; fixed alongside this, since the new logic depends on it: without it, navigating to a different date while a slow background refresh for the old one was still in flight could let that refresh land afterward and overwrite the new date with stale data for the one just left.
...
- **Android: server sync failed on pull with "ON CONFLICT clause does not match any PRIMARY KEY or UNIQUE constraint" once the #76 multi-session migration had run** ([#82](https://github.com/TraceApps/lifttrace/issues/82), diagnosed by @kgenerozov). The #76 migration widened the native `workout_tombstones` primary key to include `workout_id`, but the pull-side upsert in `sync.js` still targeted the pre-#76 five-column key, so every pull with a tombstone in it threw. A second, quieter bug rode along: tombstones were grouped and applied by date alone, so on a day with more than one session, a deletion from one session could be applied against another session's exercises instead. Both are fixed together: the upsert now targets the real primary key, and tombstones are grouped and applied per `(date, workout_id)` so a session's deletions only ever touch that session.
- **Editing a day in a program that predates uuid tagging duplicated every exercise on it, and deleting one didn't work at all** ([#85](https://github.com/TraceApps/lifttrace/issues/85), reported by @iparout). Two separate bugs: (1) an exercise list saved before per-exercise uuid identity existed has nothing to merge against, and the merge helper minted a fresh random uuid for every entry on every call, so the server's copy and the freshly re-tagged client copy could never be recognized as the same exercises, the whole list appended twice instead of merging. `mergeEntries()` now detects a fully uuid-less server side and trusts the client's resent list outright for that one bootstrap edit, exactly like the old pre-merge-system behavior, safe because a row with no identity has no concurrent-edit history to protect. (2) Independent of that: the program-day editor never tracked or sent which exercises were removed, so Option C's merge (correctly) treated a missing exercise as a possible addition from another device and kept it. Now diffs against the loaded snapshot on save, the same mechanism the Diary editor already uses.
- **Diary: starting a new workout right after clearing or deleting the current one could show it for a moment, then silently revert to the old, now-abandoned session** ([#86](https://github.com/TraceApps/lifttrace/issues/86), reported by @bauerbyter). `saveWorkout()` debounces 350ms before its network call, and a deliberate action like Clear Workout returns control to the UI immediately rather than waiting on it. If the user started a new session inside that window, its own (faster) save would apply, then the earlier debounced save would finally land and unconditionally overwrite the view with whatever it had saved for the session the user had already left. Switching sessions, deleting one, or navigating to a different date now invalidates any pending save's result before it can be applied, without touching the save itself or the promise callers may be awaiting, so nothing is lost and nothing hangs.

### Security

- Swept open Dependabot alerts, including a `browserslist` advisory disclosed today (`GHSA-73wf-gq98-2v4g`, vulnerable `<= 4.28.6`). All flagged packages, `fast-uri`, `@xmldom/xmldom`, `qs`, and `browserslist`, are already at or past their patched versions on this branch from the fix in `v1.3.0-dev01`; `npm audit` reports 0 vulnerabilities on both root and server. No change needed here.

## [1.3.0-dev01] - 2026-09-05

### Added

- **Multiple independent workout sessions per day** ([#76](https://github.com/TraceApps/lifttrace/issues/76), requested by @yoyo-san). A day is no longer limited to one workout: log a stretching routine and a separate lifting session on the same date, each tracked independently with its own exercises, sets, and completion state. A session tab strip appears once a second session exists, with a "+" to start a new one and a delete action on each tab. Loading a template or program over an already-started session now asks whether to replace it or start a new session alongside it, closing a real data-loss bug in the process: previously, loading a second template on a day with a completed workout silently deleted every exercise from the first one, with no confirmation at all. Backed by a schema change on both the server (dropped the one-row-per-day constraint on `workout_log`) and the offline Android SQLite mirror, migrated in place with a pre-migration backup so existing data survives even if the migration itself fails partway. Streaks and calendar activity still count a day as done if any session on it is complete.
- **Exercise browser display density toggle** ([#74](https://github.com/TraceApps/lifttrace/issues/74), requested by @josefelixh). The Exercises tab gets a compact/comfortable view switch next to the category filter chips (desktop widths only): compact keeps today's tight list rows, comfortable enlarges the thumbnails for easier browsing. Also fixed along the way: the exercise detail preview pane no longer scrolls away with the list behind it, it now stays put while browsing, matching the same fixed-pane behavior already used elsewhere in the app.

- **MCP server for external AI clients** ([#78](https://github.com/TraceApps/lifttrace/issues/78), requested by @bursaar). LiftTrace now speaks the [Model Context Protocol](https://modelcontextprotocol.io) at `/api/mcp`, off by default (`MCP_ENABLED=1` to turn on). Eleven tools across three phases: eight read tools (`get_workout`, `list_recent_workouts`, `get_records`, `get_exercise_progress`, `search_exercises`, `list_programs`, `get_active_program`, `get_body_stat`), two additive write tools (`log_set`, `log_body_stat`, gated on `MCP_WRITE_ENABLED=1` + `mcp:write` scope), and one destructive tool (`delete_workout`, gated on `MCP_DESTROY_ENABLED=1` + `mcp:destroy` scope + `confirm: true` on every call). Writes go through the exact same server-side merge primitives (`mergeExercises`, `mergeStatsObject`) the app's own save endpoints use, so a concurrent app save can't be clobbered mid-tool-call and a malformed payload can't write stray fields into the stored data. New **Settings → API Tokens** section (multi-user admin only — a token needs a real account to own it) issues scoped personal access tokens (`lt_pat_...`, SHA-256 hashed at rest, shown once). See the [MCP setup guide](https://traceapps.github.io/docs/lifttrace/mcp/) for wiring up Claude Desktop and other clients, and `scripts/mcp-smoke.mjs` for an end-to-end verification script to run against your own instance.
...
- **Reordering exercises no longer silently reverts.** Moving an exercise within a superset, using the ↑/↓ reorder buttons, or joining an exercise into a superset would visibly apply for a moment, then snap back once the debounced save reached the server. The server-side merge built its result from a `Map` seeded in the server's existing order and only overwrote values for matching uuids — a `Map` does not move an existing key when it's `.set()` again, so any client-side reorder was discarded while the content change (like a superset assignment) still landed. A joined exercise kept its `superset_id` but not its new array position, which also broke the diary's consecutive-run superset grouping: the exercise rendered as its own orphaned card with an unresponsive "..." menu and info button, though its set-completion toggle still worked since that doesn't depend on grouping. The merge now follows the client's array order for anything the client sent, appending only true concurrent additions from another device after it.
- **Weight and reps inputs inside a superset are readable again on a phone** ([#75](https://github.com/TraceApps/lifttrace/issues/75), diagnosed by @backmind). A superset's frame — border, connector column, side padding — took 48px out of every member's set row, and with RPE tracking on there wasn't enough left for either flexible column: both inputs could render at literally 0px wide, showing nothing while the value was still saved correctly underneath. Three changes claw the width back: the redundant "reps" unit label is gone (the column header above already reads REPS, so it cost width for nothing); the superset frame is trimmed from 48px to 12px of overhead (thinner border, connector column down from 28px to 10px, dropped right-side padding); and the weight ±step buttons are hidden specifically inside a superset, where the room is provably gone rather than merely tight — typing a weight directly still works everywhere. Standalone exercises are untouched and keep the step buttons.
- **Diary Body Stats widget and sheet no longer show "not logged" for records that exist on the server** ([#80](https://github.com/TraceApps/lifttrace/issues/80), diagnosed by @josefelixh). `GET /api/body-stats/:date` wraps the row one level deeper than the client was reading — the actual measurements sit at `data.stats.stats`, not `data.stats` — so both the widget and the full sheet always looked at the row's metadata (`id` / `user_id` / `date`) instead of the weight and measurement values. Statistics worked because its range endpoint returns already-flattened rows. Also closed the write-side version of the same bug: editing a stat while `stats` held that wrong shape would have PUT the whole misread row — including `id` / `user_id` / `date` and a duplicated nested `stats` object — back to the server, and the merge copied every key it was handed. `mergeStatsObject` now only accepts the known measurement keys, so a similar shape mismatch can't corrupt the stored data even if it recurs somewhere else later.

### Security

- Bumped `fast-uri`, `@xmldom/xmldom`, and `browserslist` (root) plus `qs` (server) for open Dependabot alerts, via a non-forcing `npm audit fix` within already-permitted lockfile ranges (no `package.json` range changes). All four are transitive build/dev-tooling or express/body-parser dependencies, none shipped in the runtime client bundle. `npm audit` reports 0 vulnerabilities on both root and server as of this release.
...
- **Desktop wide-layout pass across the main routes.** At `min-width: 1280px` (guarded against the "Force Mobile Layout" toggle), Diary, Statistics, Programs library + detail, Radio, and Exercises use a two- or three-column shell instead of the mobile stack; ExercisePicker opens as a wide modal on the same breakpoint; Settings gets a two-pane rail-and-content shell with a sliding highlight pill and a welcome-hero onboarding grid. Mobile layouts are unchanged. Verified against v1.1.2: none of these routes had any `min-width: 1024px`/`1280px` media queries.
- **Diary desktop rail with pin / hide / overlay** (mirrors NutriTrace's pattern). A right-hand rail carries This Week peek, Recent Workouts, Body Stats (summary + Log CTA), Gym Tools (plates + converter), and Workout Actions inline. Rail can be pinned in the grid, hidden (grid reclaims the column and a chevron edge-tab hovers on the right), or opened as a fixed overlay from the edge-tab; state persists to localStorage. Portaled to `document.body` so `position: fixed` resolves against the viewport rather than the `.page-transition` transform trap.
- **Spanish translation.** Complete `es` locale, exposed as `Español` in Settings → Language & Region.
- **Italian translation.** Complete `it` locale, exposed as `Italiano` in Settings → Language & Region.
...
- **Android release builds trust user-installed CA certificates** ([#58](https://github.com/TraceApps/lifttrace/issues/58), reported by @orpetor). Self-hosters serving LiftTrace over HTTPS with a private-CA cert (Bitwarden-style internal PKI, homelab step-ca, etc.) were hitting `Trust anchor for certification path not found` on the Android release build because v1.1.2's network-security config only trusted the system CA store. Added `<certificates src="user" />` alongside the system entry so any CA the user installed on the device is honored.
- **Clearing the Trace chat now asks for confirmation** ([#50](https://github.com/TraceApps/lifttrace/pull/50), thanks @backmind). v1.1.2's clearHistory tap wiped the entire conversation with no confirmation. Also fixes a z-index bug where the confirm dialog opened behind the Trace panel.
- **Statistics summary cards and plate calculator no longer show units in capitals** ([#48](https://github.com/TraceApps/lifttrace/pull/48), thanks @backmind). Reads `MAX kg` / `TOTAL min` / `TARGET WEIGHT (kg)` instead of `MAX KG` / `TOTAL MIN` / `TARGET WEIGHT (KG)`. `kg` is kilograms; `KG` is not a unit.
- **Settings → Help & Improve no longer shows mojibake where a dash belonged** ([#60](https://github.com/TraceApps/lifttrace/pull/60), thanks @backmind). Two visible strings in the diagnostics panel carried a CP1252-round-tripped em-dash; restored to U+2014.
- **Version no longer renders as `vv1.2.0`** ([#61](https://github.com/TraceApps/lifttrace/pull/61), thanks @backmind). `APP_VERSION` already carries the leading `v`, but Settings → About and the desktop sidebar were prefixing a second one.

### Security

- No new dependencies affect the security surface. `npm audit` (both root and server) reports 0 vulnerabilities and there are no open Dependabot alerts.
...
- **Logging a cardio session with a distance or a heart rate no longer fails** (#41). Both fields are `<input type="number">`, so their bound value is a number once you type in them and `null` once you clear them, but the save path called `.trim()` on them as if they were still the empty string the form starts with. Typing a distance raised `f_distance.trim is not a function`, and clearing it afterwards raised a null-property error that left the form unable to save until it was cancelled and reopened. Both fields now test for emptiness rather than assuming a string.

- **Quick-log cardio templates now show up, and pinning two sessions of the same activity no longer gives you two identical chips** (#43). `GET /api/cardio/templates` was registered below `GET /api/cardio/:date`, and Express matches in registration order, so the request was answered by the date handler with `templates` read as a date: an empty list, every time. Pinning a session persisted and reported success, but the chip row only renders when that list has entries, so the feature had never worked since it was added. The endpoint also promised one entry per activity name, most-recently-updated winning, and returned every pinned row instead.

- **Pre-caching exercise media for offline use no longer fails with "Exercise not found"** (#44). `GET /api/exercises/media-urls` was registered below `GET /api/exercises/:id`, and Express matches in registration order, so the request reached the id handler, which read `media-urls` as an id, found nothing and replied `404 Exercise not found`. The route now sits above its parameterised sibling.

- **Confirmation dialogs with two long button labels no longer overflow the dialog card** (#39). `.btn` carries `white-space: nowrap`, so a flex item can never shrink below its one-line text width. When both labels are long, as in the workout importer's `Skip duplicates` / `Replace existing` pair, the two grew past the 340px card and the left button rendered outside it. The action row now wraps.

- **The body stats sheet no longer shows its units in capitals** (#42). Field labels are uppercased by CSS, which took the unit along with the name and produced `WEIGHT (KG)`, `WAIST (CM)` and `WAIST (IN)`. Unit symbols are case-sensitive: `kg` is kilograms while `KG` is not a unit, and the same holds for `cm`, `in` and `lbs`. The symbol now opts out of the transform, so the labels read `WEIGHT (kg)` and `WAIST (cm)`.
