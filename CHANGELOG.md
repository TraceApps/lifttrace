...
- **Trace can see your progress photo history.** New `get_progress_photos` tool, so it can answer consistency questions ("how often am I taking photos?", "when was my last one?") and tie the photo record to the weight logged on those dates. Metadata only: dates, counts and weights, never image content. Sending your body photos to whichever AI provider you have configured is not something a convenience tool should do as a side effect, so if you want Trace to look at one you attach it to a message yourself, which it has always supported. Weight logged from the photo timeline already flowed to Trace without any change, since it writes to the same body-stats row `get_body_stats` and `log_body_stat` already use.
- **Progress photos.** Attach dated photos to your body-stats history and compare any two of them side by side, dragging a divider across a single frame rather than shrinking both into a split view. Lives on its own page at `/progress`, reached from Statistics (next to the body-weight chart) and from the Body Stats sheet, which also gains an "Add photo" button so a photo can be logged alongside the day's measurements. The timeline groups by month, shows the weight logged that day next to each photo when there is one, and offers a one-tap "Compare first and latest" so the comparison the feature exists for is never more than a tap away. Photos never expire: they are the long-term record, which is the opposite of what a coaching clip ([#57](https://github.com/TraceApps/lifttrace/issues/57)) would want, so the two stay separate features rather than sharing a lifetime policy. iPhone HEIC photos are converted to JPEG in the browser before upload, since Chrome, Firefox and Android WebView cannot display HEIC at all; the converter is loaded on demand, so it costs nothing for anyone who never picks one. Photos sync across devices, are included in full backups (rows and files), and are deleted from disk, not just from the database, when a photo, a user's data, or an account is removed, which is a gap the app's other uploads still have. Photos are not served from the public `/uploads` tree the way avatars and exercise media are. That tree is mounted ahead of the auth middleware so an Android WebView `<img>` can load images without an `Authorization` header, which means anything in it is readable by anyone holding the URL. That is a fine trade for a shared exercise-demo GIF and a bad one here, so progress photos are read only through an authenticated API route that looks the row up by id and checks its owner. The app fetches those bytes and renders them from an object URL, which sidesteps the WebView header problem entirely, because a `fetch()` can carry a session where an `<img>` request cannot. Cached bytes are dropped on sign-out, and they are deliberately kept out of the service worker, whose cache is shared by every account that signs in on that browser profile. One caveat worth stating plainly: in single-user mode there are no accounts, so there is no identity to check against and the entire app is open by design, the same as every other endpoint. Put it behind a reverse proxy or a VPN, as always. Tapping any photo opens a scrubber: the photo full-frame with a draggable date track under it, so you can sweep across months, or press play and watch the whole record run as a time-lapse. The track is a true date axis rather than one even step per photo, so a gap in the habit shows up as a gap. Compare answers how two specific dates differ; this answers what the whole arc looked like, which matters because any two adjacent photos look identical and the change is only visible in the sweep. Also exposed through the MCP server (`list_progress_photos`, `add_progress_photo`) and the REST API (`GET`/`POST /api/v1/body-stats/photos`) under the existing `mcp:read`/`mcp:write` scopes, and a new `progress_photo.logged` webhook fires no matter which of the three attaches the photo.
