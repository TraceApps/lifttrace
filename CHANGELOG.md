...
- **The bottom nav's active-tab pill now respects your accent color.** Its background gradient blended the theme-aware `--accent` into a hardcoded orange (`rgba(255,116,51,...)`) instead of `--accent-2`, so any non-default accent (purple, mint, blue, ...) showed an orange-tinted pill clashing with the rest of the UI.
