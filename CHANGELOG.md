...
- **LiftTrace on your wrist.** A Wear OS app for watches running Wear OS 3 and up: it opens on the set you are in front of, with the weight and reps already filled in from your programme or from the set before it, so logging one is a tap. The rest timer then counts down on your wrist and buzzes when it is over, which is the whole reason to have it there rather than on a phone in a locker. A tile puts the current set one swipe from the watch face and a complication puts the set count on the face itself. The watch talks to your server itself rather than through the phone, so it keeps working in a basement gym: what you log with no signal is held on the watch and goes up on its own, sent as the whole day so a session logged on both the watch and the phone merges set by set. Nothing is typed on the watch, and nothing is set up there either: the phone hands over your server address and a token when you sign in, and takes them away when you sign out. It follows your own rest length, auto-start and weight unit. Planning a session, the exercise catalogue and statistics stay on the phone, where there is room for them.
