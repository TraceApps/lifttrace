...
- **Film a set and have your coach review it** ([#57](https://github.com/TraceApps/lifttrace/issues/57), requested by @backmind). The filming half of coaching happened entirely outside the app: record the set, send it to yourself, forward it on. A set can now carry a clip. Film it in the app, where it records at a size that actually uploads, or pick one you already have. A filmed set is marked in the Diary, on the set row and on the card when it is collapsed, and tapping it plays the clip back. Your coach sees the same clip on the session, watches it, pauses where the problem is, and leaves a note stamped at that moment, so tapping "0:04" in your Diary jumps the video straight there. You can reply to the note without leaving the player. Clips are private in a way the rest of the uploads folder is not: they are never readable by URL, only you and your own coach can open one, and an admin is nobody's exception. They sync between your devices, ride along in a full backup, and go from disk the moment you delete one, along with your account if you ever delete that. Nothing expires on its own, because deleting your training footage on a schedule you did not choose is worse than the space it costs, so Settings, Workout shows what your clips are using and clears out anything older than a month or six months when you ask. Attaching one needs a connection, since a video is far too big to hold in the offline queue; everything else in the diary still works without one.
- **Reading your training over a date range, for AI clients and scripts** ([#93](https://github.com/TraceApps/lifttrace/pull/93), by @kgenerozov). Trace and anything else using the MCP server or the REST API could read one day at a time, or the last 90 days, and nothing else, so a question like "how has my bench moved since March" meant one call per day. Two new reads answer a whole range at once, `get_workouts` with full set detail and `get_body_stats` for weigh-ins and measurements, and recent workouts, records and per-exercise progress now take the same bounds. Giving one bound leaves the other open, so a start date on its own reads forward, planned sessions included. Dates are checked properly, including a 31st of February. Full-detail reads answer the most recent 30 sessions by default and say when there were more, rather than returning a year of sets in one reply. Records read over a range are the best lifts within those dates, which the tool now says plainly so an assistant doesn't report a March best as an all-time record. The same filters are on the REST API, with new `GET /api/v1/workouts` and `GET /api/v1/body-stats` endpoints.
- **LiftTrace on your wrist.** A Wear OS app for watches running Wear OS 3 and up. It opens on the set you are in front of, with the weight and reps already filled in from your programme or from the set before it, so logging one is a tap. You can also go through the session yourself: open any exercise, open any set, change the weight, the reps (per side when your phone records them that way) or the length of a hold, mark a set done, put one back to not done if you tapped the wrong one, and add a set when you have one more in you. The session timer is on the watch too, and it is the same timer the phone keeps: start it, pause it or stop it from either, and both show the same number. Stopping it on the watch writes the session's length down, so a workout timed entirely from your wrist still records how long it took. It keeps counting with the phone in a locker, and whichever device you last touched is the one that counts. Under the exercise's name it tells you what you did of that lift last time, so the one number people take their phone out for is already on the wrist. A hold counts down on the watch and logs itself at the end. A superset alternates the way you actually train it, handing you A1, A2, A1, A2 rather than every set of one and then the other. The rest timer runs on your wrist and buzzes when it is over, started by itself after a set or by hand between exercises, which is the whole reason to have it there rather than on a phone in a locker; it follows your own rest settings, a programme's per-exercise rest, and the same rules the phone uses (nothing after a warm-up, and a superset rests once the round is over rather than between the two exercises). The buzz is a real alarm rather than something the app has to stay awake for, so it happens with your wrist down and the watch face back, and a rest still running is there when you open the app again. A tile puts the current set one swipe from the watch face and a complication puts the set count on the face itself.

  The watch talks to your server itself rather than through the phone, so it keeps working in a basement gym: what you change with no signal is held on the watch and goes up on its own. Sending reads the day back first and replays your changes onto it, exactly as the phone does, so a session's name, notes, length, programme and anything logged on the phone in the meantime all survive a set logged on the watch, and a session you deleted on the phone is never quietly brought back. While the app is open it also checks back every so often, so a set ticked off on the phone shows up on the wrist. Nothing is typed on the watch, and nothing is set up there either: the phone hands over your server address and a token when you sign in, and takes them away when you sign out. It follows your own rest length, auto-start and weight unit. Planning a session, the exercise catalogue, statistics, cardio and body stats stay on the phone, where there is room for them, and a coach's notes are never copied to the watch.

  The crown scrolls the list, as it does everywhere else on a watch. Tap a number and it takes the crown: the number is outlined and turning moves it, tapping it again gives the crown back. The weight, the reps, each side of a unilateral lift and the length of a hold can all be dialled that way, rather than the crown always meaning weight.

  A block of work that is entirely done moves under a **Completed** heading at the bottom, keeping the order you did it in, so the end of a long session is not six finished cards to scroll past before the one you are up to. A superset only moves when the whole superset is done, since half of one is still work, and reopening a set brings it straight back up.

  **The session can be finished from the wrist.** The length comes off the running clock, the day is marked done and it goes up with everything else, queued like any other change so a gym with no signal still ends the session. With sets still unfinished it asks first and says how many, and they stay as they are.

  A running rest now shows on the watch face and in the launcher while it counts, without the app open, with the system drawing the countdown so nothing wakes up to tick it, and it says "Rest over" on the wrist when it rings rather than buzzing about nothing. On the session screen the rest counts down in its own colour, green while there is time, amber when it is getting on, red at the death, instead of a row that only said "back to the timer".

  **The rest is shared with the phone.** Start, extend or skip it on either device and the other follows. The phone only sends one to a watch whose app has been opened in the last half hour, so a watch on a charger in another room is not woken for a rest and does not buzz about one, and when the watch does have it the phone keeps its own notification to itself rather than mirroring an unlabelled copy to your wrist.

- **The web app keeps working without a connection** ([#211](https://github.com/TraceApps/nutritrace/issues/211) in NutriTrace, the same idea here). In a gym with no signal the app still opens, today's workout is there, and you can keep logging: sets, reps, weight, notes, a whole session, body stats, your own exercises, and settings. What you change is held in the browser and goes up on its own when the connection comes back, replayed against the same routes the Android app replays its own queue against, so a session changed on two devices merges set by set rather than one copy overwriting the other. The menu button shows an amber cloud while anything is waiting, and red if your server refuses it. Signing out sends what is waiting first, then clears the copy held in the browser. Nothing is sent while you are offline, and the queue is cleared only once the server has confirmed it. Programs, templates, your exercise catalogue, recent workouts and Statistics all read from what this browser has already seen. Cardio is in too: log, edit or delete a run with no signal, on a day you have never even opened, and it is there when you reconnect. So is starting a program or moving its week, since that happens at the gym. Signing out sends what is waiting first, and if it cannot reach your server it asks before discarding it rather than clearing it silently. Coaching works offline too: write a note on a member's session, reply to one, mark notes read, and prescribe work, all with no signal. Progress photos work offline too: take one in a gym with no signal and it travels inside the entry, becoming an ordinary file on your server the moment the queue goes up, kept out of the public uploads tree exactly as before. A photo too large to hold says so rather than being lost. Your profile works offline too, picture included: choose one in a gym with no signal and it travels with the profile, becoming an ordinary file on your server when the queue goes up. A picture or an animation on one of your own exercises works offline too, the same way; a video still waits, since it is far too big to keep in a row. Still needs a connection: exercise videos, the exercise catalogue imports, creating or editing a program or template, Trace, adding or removing a member, assigning a program, and anything admin. If your server refuses one of your changes when it finally reaches it, that change is set aside and named in plain words rather than silently dropped or left blocking everything behind it, and it is written to Settings, Diagnostics so you can find it later. A server that is merely busy or struggling, or a session that has expired, is retried instead: your work is never thrown away because a cookie timed out. Deliberately not built on Background Sync, which Safari does not have, so an iPhone behaves the same as everything else.

### Fixed
- **A filmed set shows a still, not a giant play symbol.** The review screen and the coach's player handed Android a video with no poster, so it drew its own play glyph blown up to the size of the box until you pressed play. Both now show a frame from the clip itself: the moment you stopped filming, or a frame decoded from the file you picked.
...
- **The bottom nav's active-tab pill now follows your accent colour** ([#114](https://github.com/TraceApps/lifttrace/pull/114), found and fixed by @benniemosher). The second half of its gradient was a hardcoded orange, the default accent's own colour, so picking purple, mint, blue or any other accent left an orange highlight sitting behind accent-coloured text. The default theme looks exactly as it did.
- **The hold timer's "open this hold's date" button now loads that day.** It moved the date header without fetching the day behind it, so with a hold running for another date, tapping it left the session you were already looking at on screen under the new date. Same cause as the This Week strip above, and the last place in the Diary with it.
- **Tapping a day in the Diary's "This Week" strip, or a row in "Recent," now actually loads that day** ([#111](https://github.com/TraceApps/lifttrace/pull/111), found and fixed by @benniemosher). Both jumped the date header and week-strip selection to the tapped date but never fetched its workout, the session panel kept showing whatever was already loaded (often "Nothing logged yet" for a day that has one), since only `loadWorkout()` fetches and populates it. The `<`/`>` day-navigation arrows already did this correctly; the two click handlers now go through the same path.
- **Moving an exercise past a superset no longer breaks the superset up.** The up and down arrows swapped one row with the next, so an exercise sitting below a pairing swapped places with the pairing's second exercise and pulled it out of the group, leaving it stranded on its own underneath. The arrows now move whole blocks, the way dragging already did: a superset travels as a unit, and anything moving past one steps over the whole thing. Reported in testing.
...
- **Something deleted while online stays deleted when the connection goes.** The copy this browser keeps was only tidied after a change had to wait its turn, not after one that went straight through, so a deleted progress photo reappeared the moment you were offline again. Reported in testing.

- **A progress photo added with no connection now shows straight away and is never refused for its size.** Three things were wrong at once: the photo was kept at its full size and your server turned it away with "request entity too large" when it finally arrived; it did not appear on the timeline while it waited, so it looked as though nothing had happened; and the part of the app that keeps a photo was fetched from your server at the very moment there was nothing to fetch from. A photo is now scaled to something a request comfortably carries, appears on the timeline the moment you add it, and the routes that accept one allow room for it.
...
- **Adding a progress photo with no connection no longer says "Could not save photo".** The photo was being kept and did reach the server afterwards, but the screen was told about it in the wrong shape and reported a failure that had not happened. Every change made offline is now answered the way your server would answer it.
...
- **A coach's note stays on the exercise it was written about** even when the member reorders or removes exercises afterwards. Notes were pinned to an exercise's position in the session, so a note could end up against the wrong lift; they now follow the exercise itself. Notes written before this keep working as they did.
...
- **Outgoing webhooks** ([#79](https://github.com/TraceApps/lifttrace/issues/79), requested by @bursaar). Point LiftTrace at a URL and it fires a signed POST when a workout is completed, a personal record is set, a program advances a week, a body stat is logged or a progress photo is added. For n8n, Home Assistant or anything else, without polling. Off by default (`WEBHOOKS_ENABLED=1`). Each delivery is signed with HMAC-SHA256 and retried up to three times, the secret is encrypted at rest and shown once, and target URLs are checked against the same protection used for the radio proxy, so private, loopback and cloud-metadata addresses are blocked and redirects are not followed. See `docs/webhooks.md`.
- **Sync status in the sidebar.** On Android connected to a server, the sidebar shows Synced, Syncing, Offline or Can't reach the server. Colour means one thing everywhere now: amber when the phone simply has no network, so nothing is lost, and red when the server is reachable but the sync is failing.
- **Exercise browser display density toggle** ([#74](https://github.com/TraceApps/lifttrace/issues/74), requested by @josefelixh). A compact or comfortable view switch next to the category filters at desktop widths. The exercise detail pane also stays put while you browse instead of scrolling away.
...
- **Statistics desktop rail headings and comparison cards now translate** ([#83](https://github.com/TraceApps/lifttrace/pull/83)), along with the API Tokens intro link ([#84](https://github.com/TraceApps/lifttrace/pull/84)) and the email settings environment banner, all of which stayed English in every translated locale.
...
- **A stale session id could spawn a duplicate session on every set edit, and deleting a session didn't stick** ([#87](https://github.com/TraceApps/lifttrace/issues/87), diagnosed by @kgenerozov). Deleting a workout is now recorded rather than erased, so other devices learn about it instead of pushing the workout back.
- **Sync failed on pull with an "ON CONFLICT" error after the multi-session update** ([#82](https://github.com/TraceApps/lifttrace/issues/82), diagnosed by @kgenerozov). A deletion on a day with more than one session could also be applied to the wrong session.
- **Tapping a weight or reps field brought up the system text toolbar over the set row** ([#95](https://github.com/TraceApps/lifttrace/issues/95), reported and diagnosed by @kgenerozov). Nothing is selected now: the value dims when you tap and the first digit replaces it. Backspace, a second tap and the arrow keys still edit normally.
- **Reminders ignored the times set in Settings, the streak reminder never fired, and the weekly summary arrived every day** ([#97](https://github.com/TraceApps/lifttrace/pull/97), found and fixed by @backmind). The on-device reminders read setting names the app never stores, so every one fell back to a default, and they were scheduled as repeating alarms that opening the app kept pushing a day out. They now read the real names and fire at the time of day you chose.
- **Trace's Base URL and API Key could not be saved in portrait, and AI settings drifted from the web** ([#94](https://github.com/TraceApps/lifttrace/issues/94), reported by @kgenerozov). The Save buttons sat past the edge of the screen; both fields now save when you leave them. A setting changed while offline no longer stays marked as waiting to sync for good.
...
- **A workout couldn't be added back after Clear Workout** ([#99](https://github.com/TraceApps/lifttrace/issues/99), reported by @LeVraiRoiDHyrule). Exercises copied into a day now get ids of their own, so a template can be loaded as often as you like. Days already affected work again with no change to your data.
- **Taking an exercise out of a superset made the whole superset disappear** ([#96](https://github.com/TraceApps/lifttrace/pull/96), found and fixed by @backmind). The exercise now moves below the block, so the members left behind stay together.
- **Starting a new workout right after clearing or deleting one could revert to the old session** ([#86](https://github.com/TraceApps/lifttrace/issues/86), reported by @bauerbyter).
...
- **Weight and reps inputs inside a superset are readable again on a phone** ([#75](https://github.com/TraceApps/lifttrace/issues/75), diagnosed by @backmind). With RPE on, both could render at zero width, showing nothing while the value was saved correctly underneath.
- **Editing a day in an older program duplicated every exercise on it, and deleting one didn't work** ([#85](https://github.com/TraceApps/lifttrace/issues/85), reported by @iparout).
- **Delete Workout could leave the workout in place on a day with more than one session.**
- **The bottom of the side columns was hidden behind the Radio player** ([#104](https://github.com/TraceApps/lifttrace/issues/104), reported by @LeVraiRoiDHyrule). On wider screens, rows like Delete Workout could sit behind the player or the rest timer. The columns now leave room for them.
- **The Diary Body Stats widget and sheet showed "not logged" for records that exist on the server** ([#80](https://github.com/TraceApps/lifttrace/issues/80), diagnosed by @josefelixh).
...
- **Radio couldn't play from Jellyfin 12** ([#100](https://github.com/TraceApps/lifttrace/issues/100), reported by @LeVraiRoiDHyrule). Radio signs in the way current Jellyfin expects and falls back to the older way for earlier servers. The Test button signs in for real, and a saved sign-in that stops working renews itself.
...
- **Uploaded files are stored and served more strictly.** An upload's extension now comes from what kind of file it actually is rather than its name, and everything under `/uploads` is served with `X-Content-Type-Options: nosniff` and a sandboxing content policy.
- **Webhook targets are checked for private and cloud-metadata addresses** on every attempt, including retries, and redirects are not followed.
- Dependency advisories cleared over the cycle: `multer`, `nodemailer`, `adm-zip`, `devalue`, `fast-uri`, `@xmldom/xmldom`, `qs` and `browserslist`. `npm audit` reports 0 vulnerabilities for both the app and the server, and every open Dependabot alert is already patched on this release.
