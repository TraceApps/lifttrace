...
- **Deleted workouts still counted in Statistics and personal records.** Deleting a workout removed it from the Diary but not from the numbers built on it, so its sets could still hold a personal record, add to volume and charts, and push the "All" range back to its date. Records over MCP, the REST API and the `pr.set` webhook had the same gap, and so did workout, rest day and streak reminders (a deleted workout counted as training that day), the weekly summary, program week progress, exercise usage sorting, and the coach views of a member's recent workouts, streak and prescriptions. Deleted workouts are now left out of all of them. Over MCP, `delete_workout` now deletes the way the app does, so other devices see it go instead of syncing it back, and logging a set to a date whose workout you deleted starts a fresh session rather than restoring the deleted one. Re-importing a workout you deleted no longer skips it as a duplicate.
