...
- **Progress photos.** Attach dated photos to your body-stats history and compare any two of them side by side, dragging a divider across a single frame rather than shrinking both into a split view. Lives on its own page at `/progress`, reached from Statistics (next to the body-weight chart) and from the Body Stats sheet, which also gains an "Add photo" button so a photo can be logged alongside the day's measurements. The timeline groups by month, shows the weight logged that day next to each photo when there is one, and offers a one-tap "Compare first and latest" so the comparison the feature exists for is never more than a tap away. Photos never expire: they are the long-term record, which is the opposite of what a coaching clip ([#57](https://github.com/TraceApps/lifttrace/issues/57)) would want, so the two stay separate features rather than sharing a lifetime policy. iPhone HEIC photos are converted to JPEG in the browser before upload, since Chrome, Firefox and Android WebView cannot display HEIC at all; the converter is loaded on demand, so it costs nothing for anyone who never picks one. Photos sync across devices, are included in full backups (rows and files), and are deleted from disk, not just from the database, when a photo, a user's data, or an account is removed, which is a gap the app's other uploads still have. One thing to know before turning this on: image files under `/uploads` are served before the auth middleware, deliberately, because an Android WebView `<img>` tag cannot send an `Authorization` header. That means a progress photo is viewable by anyone holding its URL, signed in or not, protected only by its random filename. That tradeoff already applied to avatars and exercise media; this extends it to a more sensitive category, so it is called out rather than left to be discovered. Making it airtight needs signed or expiring URLs across every upload in the app, which is tracked separately. The weight logged that day rides along beside each photo, now with its unit and, in the scrubber, the change since your first weighed photo, which is the number that actually means something while sweeping across months. If a photo's date has no weight logged, you are offered one inline right there rather than being sent to the diary for it; it writes to the same body-stats row every other weight surface uses, merging into that day rather than replacing it, so a waist or body-fat figure already logged is never clobbered. Captures default to today but the date is settable, so a camera roll of older photos can be backfilled without walking the diary one day at a time. Tapping any photo opens a scrubber: the photo full-frame with a draggable date track under it, so you can sweep across months, or press play and watch the whole record run as a time-lapse. The track is a true date axis rather than one even step per photo, so a gap in the habit shows up as a gap. Compare answers how two specific dates differ; this answers what the whole arc looked like, which matters because any two adjacent photos look identical and the change is only visible in the sweep. Also exposed through the MCP server (`list_progress_photos`, `add_progress_photo`) and the REST API (`GET`/`POST /api/v1/body-stats/photos`) under the existing `mcp:read`/`mcp:write` scopes, and a new `progress_photo.logged` webhook fires no matter which of the three attaches the photo.
