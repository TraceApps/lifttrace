/**
 * server/routes/mcp.js — Model Context Protocol endpoint (issue #78).
 *
 * Mounted at /api/mcp when MCP_ENABLED=1 in the server env. Off by
 * default, so no existing user sees any change. Write tools (log_set,
 * log_body_stat) additionally require MCP_WRITE_ENABLED=1 on the server
 * AND the calling token holding the `mcp:write` scope. The destructive
 * tool (delete_workout) requires MCP_DESTROY_ENABLED=1 AND `mcp:destroy`
 * scope AND the call to include confirm=true. Any missing gate and the
 * corresponding tools don't appear in tools/list.
 *
 ...
 * verbs don't apply). Auth is bearer-token via the api_tokens table
 * with scope 'mcp:read'. Origin is validated as a DNS-rebinding
 ...
 * limit budget. Ported from NutriTrace's server/routes/mcp.js —
 * security logic is fully generic, only the tool set differs.
 */
...
router.get('/', (req, res) => {
  res.status(405).json({
    ...
    error: {
      code: -32000,
      message: 'GET not supported: LiftTrace runs MCP in stateless mode. Use POST.',
    },
  });
});
