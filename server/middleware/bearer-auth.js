 ...
 *
 * Bearer-token auth for external integrations (/api/mcp). Validates the
 * Authorization header against api_tokens, attaches { user, token } to
 * the request, and rate-limits per token. Ported from NutriTrace's
 * middleware/bearer-auth.js — fully generic, no NT-specific logic.
 */
 ...
 *   router.get('/foo', bearerAuth, requireScope('mcp:read'), handler)
 */
