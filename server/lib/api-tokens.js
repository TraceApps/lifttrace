 ...
 *
 * Personal access token management for external integrations (issue #78:
 * MCP server). Tokens are SHA-256 hashed at rest. Raw values are returned
 * to the caller exactly once at creation; lookup is hash-based.
 *
 * Token format: `lt_pat_<43 base64url chars>`. The `lt_pat_` prefix makes
 * leaked tokens recognizable in logs and credential scanners. Ported from
 * NutriTrace's server/lib/api-tokens.js — same shape, LT-specific scopes.
 */
import { randomBytes, createHash } from 'crypto';
import db from '../db.js';

const TOKEN_PREFIX = 'lt_pat_';
...
export const KNOWN_SCOPES = new Set([
  ...
  // each destructive tool call to include an explicit `confirm: true`
  // argument. Kept separate from mcp:write so an admin can grant "log
  // stuff" without granting "delete stuff that's already there".
  'mcp:destroy',
]);
