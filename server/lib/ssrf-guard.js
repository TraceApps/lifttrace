 ...
 * server-side. Extracted from server/routes/radio-proxy.js (which
 * originally carried its own copy) so both the radio proxy and outgoing
 * webhooks (issue #79) share one implementation: security-critical logic
 * like this should have exactly one place to fix if a bypass is ever
 * found, rather than two copies that can silently drift.
 *
 * radio-proxy.js keeps a same-name, same-signature local wrapper around
 * assertSafeUrl so its own call sites needed zero changes when this was
 * extracted: behavior there is unchanged, just relocated.
 *
 * Tiered defaults:
 *   - Link-local / IPv6 link-local / cloud-metadata (169.254.169.254):
 *     ALWAYS blocked. Never legitimate for either caller.
 *   - Loopback + RFC1918 + IPv6 ULA: blocked by default; each caller
 *     passes its own `allowPrivate` flag (radio-proxy.js:
 *     ALLOW_PRIVATE_RADIO_URLS, webhooks: ALLOW_PRIVATE_WEBHOOK_URLS) so
 *     a self-hoster can opt in independently per feature.
 */
 ...
 * follows it. Accepted as residual risk here, same acceptance
 * radio-proxy.js's original docstring already made for its own use
 * case; webhook delivery re-validates via this same function
 * immediately before every send attempt (not just once at creation
 * time), which narrows but does not eliminate the window.
 */
