 ...
 *
 * Outgoing webhooks (issue #79). When a subscribed event happens
 * (workout completed, a PR set, a program week advanced, a body stat
 * logged), fire a signed HTTP POST to each of the user's enabled
 * webhooks subscribed to that event. Off by default (WEBHOOKS_ENABLED=1
 * to turn on).
 *
 ...
 * user waiting for a real workout completion or PR. Bypasses the
 * enabled/subscribed-events filtering, a user testing a webhook wants
 * to know the URL and secret actually work, regardless of whether the
 * row is currently enabled or which events it's subscribed to.
 */
export async function sendTestWebhook({ userId, id }) {
  ...
  await _deliverWithRetry(row, 'test', { message: 'This is a test event from LiftTrace.' });
